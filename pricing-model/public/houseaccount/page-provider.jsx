...
const RECENT_WORK_WHEN = ["2 weeks ago", "last month", "3 weeks ago", "last week"];

const ProviderGallery = ({ p }) => {
  const work = SERVICES.filter((s) => s.providerId === p.id).slice(0, 2);
  ...
  return (
    <section className="prov-gallery">
      ...
      <div className="gal-grid">
        {work.map((s, i) => (
          <figure key={s.id} className="work-frame">
            <div className="work-img"><CatTile category={s.category} iconSize={34} /></div>
            <figcaption>
              <strong>{s.name}</strong>
              <span className="muted">{catName(s.category)} · completed {RECENT_WORK_WHEN[i % RECENT_WORK_WHEN.length]}</span>
            </figcaption>
          </figure>
        ))}
      </div>
...
const CompanyProfile = ({ p, nav, addToCart, toast }) => (
  <main className="ha-page provider company-layout">
    <div className="banner">
      {p.storefrontImg
        ? <image-slot id={`${p.id}-storefront`} shape="rect" src={p.storefrontImg}></image-slot>
        : <div className="banner-fallback" style={{ "--bh": p.avatarHue }}></div>}
    </div>
    <div className="inner">
      <div className="ident">
      ...
      <div className="cols">
        <aside className="side-col left">
          <div className="ha-panel persona">
            {p.photo
              ? <image-slot id={`${p.id}-portrait`} shape="circle" src={p.photo}></image-slot>
              : <Avatar provider={p} size={132} />}
            <ProviderKind provider={p} />
            <h1>{p.name}</h1>
            <p className="tagline">{p.tagline}</p>
            <RatingLine rating={p.rating} count={p.reviewCount} size={15} />
