        ...
        <div className="pdp three-col">
          <div className="media">
            <div className="main-shot">
              <image-slot id={`svc-${s.id}-hero`} shape="rounded" radius="16"
                placeholder={`Drop a photo of "${s.name}" in action`}></image-slot>
              <CatTile category={s.category} iconSize={52} />
            </div>
            <div className="thumbs">
              {[1, 2, 3].map((n) => (
                <image-slot key={n} id={`svc-${s.id}-t${n}`} shape="rounded" radius="10" placeholder="+ photo"></image-slot>
              ))}
            </div>
